...
FDCAN_TxHeaderTypeDef txtHeader;
uint8_t txtData[8];
FDCAN_FilterTypeDef sFilterConfig;
volatile uint8_t SharedData[8];
volatile uint8_t frame_100h[8];
volatile uint8_t frame_102h[8];
volatile uint8_t frame_103h[8];
volatile uint8_t frame_105h[8];
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan1, uint32_t RxFifo0ITs)
{
	...
	//	// Switch to the higher priority task if needed
	//	portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
	//

	FDCAN_RxHeaderTypeDef rxtHeader;
	uint8_t rxData[8];
	uint8_t rxData_103[8];
	...
	if((RxFifo0ITs & FDCAN_IT_RX_FIFO0_NEW_MESSAGE) != RESET)
	{
		...
		if (HAL_FDCAN_GetRxMessage(hfdcan1, FDCAN_RX_FIFO0, &rxtHeader, rxData) != HAL_OK)
		{
			/* Reception Error */
			Error_Handler();
		}

		if( rxtHeader.Identifier == 0x100)
		{
			for(i= 0;i<8;i++)
				frame_100h[i] = rxData[i];
		}

		if( rxtHeader.Identifier == 0x101)
		{

			for(i= 0;i<8;i++)
				SharedData[i] = rxData[i];
		}
		if( rxtHeader.Identifier == 0x102)
		{

			for(i= 0;i<8;i++)
				frame_102h[i] = rxData[i];
		}
		if( rxtHeader.Identifier == 0x103)
		{
			for(i= 0;i<8;i++)
				frame_103h[i] = rxData[i];
		}

		if( rxtHeader.Identifier == 0x105)
		{
			for(i= 0;i<8;i++)
				frame_105h[i] = rxData[i];
		}
	...
	//xSemaphoreGiveFromISR(sharedDataMutex, NULL);
}
...
void can_init(void)
{
	...
	hfdcan1.Instance =  ((FDCAN_GlobalTypeDef *) FDCAN1_BASE_NS);
	hfdcan1.Init.ClockDivider = ((uint32_t)0x00000000U);//div 1
	hfdcan1.Init.FrameFormat = ((uint32_t)0x00000000U);//classic
	hfdcan1.Init.Mode = ((uint32_t)0x00000000U);//normal
...
void can_frame_gen()
{
	...
	txtHeader.Identifier =0x100;
	txtHeader.IdType = FDCAN_STANDARD_ID;
	txtHeader.TxFrameType = FDCAN_DATA_FRAME;

	txtHeader.DataLength = 8;
	txtHeader.ErrorStateIndicator = FDCAN_ESI_PASSIVE;
	txtHeader.BitRateSwitch = FDCAN_BRS_OFF;
	txtHeader.FDFormat = FDCAN_FD_CAN;
	txtHeader.TxEventFifoControl = FDCAN_NO_TX_EVENTS;
	txtHeader.MessageMarker = 0x02;
...
void can_send()
{
	...
	HAL_Delay(100);
	if (HAL_FDCAN_AddMessageToTxFifoQ(&hfdcan1, &txtHeader, txtData) != HAL_OK)
	{
		Error_Handler();
	}
